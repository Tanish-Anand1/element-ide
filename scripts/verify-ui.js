...
try {
  ...
  const resized = await evaluate(`(() => { const splitter = document.querySelector('#splitter'); splitter.dispatchEvent(new KeyboardEvent('keydown', { key: 'ArrowDown', bubbles: true })); return splitter.getAttribute('aria-valuenow'); })()`); assert.equal(resized, '49');
  ...
  await evaluate(`window.monaco.editor.getEditors()[0].getModel().setValue('Changed from Monaco live preview')`);
